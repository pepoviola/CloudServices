...
function log(text)
{
   get_id('msgBox').innerHTML += "<br />" + text;
   get_id('msgBox').scrollTop = get_id('msgBox').scrollHeight;
}
...
function createPluginObject(parentId)
{
   ...
   get_id(parentId).appendChild(obj);
   return get_id('vmrc');
}
...
function init()
{
   ...
   for (var e in enumDefs) {
      ...
      for (var i = 0; i < keys.length; i++) {
         ...
         if (get_id(propertyName)) {
            var opt = new Option(shortkey, value);
            opt.id = shortkey;
            get_id(propertyName)[get_id(propertyName).length] = opt;
         }
      }
   }
}
...
function connect() {
   ...
   var thumb = $V('connect_thumbprint');
   var allowSSLErrors = get_id('connect_allow_ssl_errors').checked;
   var ticket = $V('connect_ticket');
   var user = $V('connect_username');
   var pass = $V('connect_password');
   var vmid = $V('connect_vmid');
   var datacenter = $V('connect_datacenter');
   var vmPath = $V('connect_vmpath');

   try {
      //console.log(host, thumb, allowSSLErrors,
...
function setFullscreen() {
   var fs = get_id('fs_value').checked;
...
function setInputRelease() {
   var release = get_id('input_value').checked;
