...
vmware.installer = (function () {
    ...
    var isVmrcInstalled = function (parent) {
        ...
        if (typeof vmrc.getVersion === 'undefined'){
            //typeof vmrc.getSupportedApi === 'undefined') {
    ...
    var installVmrcIfNeeded = function (baseAddress) {
        ...
        if (!isVmrcInstalled($('body'))) {
            var installer_name = chooseInstaller();
            var installer = baseAddress + installer_name;

            //var message = 'Please <a href="{0}">download the installer</a>.'.localize(installer);
            var message = 'Plugin no instalado, por favor <a href="'+installer+'" target="_blank">descargar el instalador</a>. Luego de instalar recargar el navegador';
            //console.log(message);
            template = '<div class="alert alert-error"><button type="button" class="close" data-dismiss="alert">&times;</button>%mensaje%</div>'.replace('%mensaje%',message);
    ...
    that.createControl = createControl;
